...
export default class Ins4 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// smallPaper
		const smallPaper = this.add.image(211, 311, "smallPaper");
		smallPaper.scaleX = 0.20019487082473372;
		smallPaper.scaleY = 0.20019487082473372;

		// text
		const text = this.add.text(206, 300, "", {});
		text.scaleX = 0.9897272814457403;
		text.scaleY = 0.9897272814457403;
		text.setOrigin(0.5, 0.5);
		text.text = "You’ll get points based on how quickly you can react to the bar appearing on screen, as well as combos.";
		text.setStyle({ "color": "#000000ff", "fontFamily": "CeraProRegular", "fontSize": "28px" });
		text.setWordWrapWidth(222);

		// next2
		const next2 = this.add.image(285, 506, "smallnext");
		next2.scaleX = 0.18;
		next2.scaleY = 0.18;

		// skip2
		const skip2 = this.add.image(127, 502, "smallskip");
		skip2.scaleX = 0.18;
		skip2.scaleY = 0.18;
	...
	create() {
		...
		this.next2.on('pointerdown', () => {
			this.socket.off("phone_disconnected");
			this.buttonClick.play();
			this.scene.start('Ins5', { socket: this.socket, roomCode: this.roomCode });
		});
