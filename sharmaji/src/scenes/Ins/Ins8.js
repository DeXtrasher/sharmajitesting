...
export default class Ins8 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// momCharacter
		const momCharacter = this.physics.add.sprite(-130, 350, "momwalksleft", 0);
		...
		// smallPaper
		const smallPaper = this.add.image(452, 246, "smallPaper");
		smallPaper.scaleX = 0.20019487082473372;
		smallPaper.scaleY = 0.20019487082473372;

		// text
		const text = this.add.text(454, 235, "", {});
		text.scaleX = 0.9897272814457403;
		text.scaleY = 0.9897272814457403;
		text.setOrigin(0.5, 0.5);
		text.text = "Try not to hit him when your wife is in the room. (Ideally, don’t hit him at all, perhaps?)";
		text.setStyle({ "color": "#000000ff", "fontFamily": "CeraProRegular", "fontSize": "28px" });
		text.setWordWrapWidth(262);

		// next2
		const next2 = this.add.image(527, 439, "smallnext");
		next2.scaleX = 0.18;
		next2.scaleY = 0.18;

		// skip2
		const skip2 = this.add.image(369, 435, "smallskip");
		skip2.scaleX = 0.18;
		skip2.scaleY = 0.18;
	...
	create() {
		...
		this.next2.on('pointerdown', () => {
			...
			this.buttonClick.play();
			this.momFootsteps.stop();
			this.scene.start('Ins9', { socket: this.socket, roomCode: this.roomCode });
		});
